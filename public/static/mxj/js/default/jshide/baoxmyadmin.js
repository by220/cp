...
function bao() {
	...
	$.ajax({
		...
		success: function(mm) {
			...
			m = mm['zbao'];
			str += "<tr><th colspan=9 class='bt'>总报表</th></tr><tr><th>公司</th><th>注数</th><th>会员下注</th><th>会员退水</th><th>贡献率</th><th>占比</th><th>集团占成</th><th>退水</th><th>集团收付</th></tr>";
			for (i = 0; i < zbl; i++) {
				if (Number(m[i]['userzs']) == 0) continue;
				str += "<tr gid='all'>";
				str += "<td><a href='javascript:void(0);' class='u' fid=0 ifagent='" + m[i]['ifagent'] + "' uid='" + m[i]['userid'] + "' layer='" + m[i]['layer'] + "'   fly='" + m[i]['fly'] + "'>" + m[i]['username'] + "</a></td>";
				str += "<td>" + m[i]['userzs'] + "</td>";
				str += "<td>" + m[i]['userje'] + "</td>";
				str += "<td>" + m[i]['userpoints'] + "</td>";
					str += "<td class='gxl'>"+baifen(m[i]['userje'],m[i]['upje'])+"</td>";
					str += "<td class='zanbi' v2='"+m[i]['upje']+"'></td>";
				str += "<td>" + m[i]['upje'] + "</td>";
				str += "<td>" + m[i]['points'] + "</td>";
				str += "<td class='sy'>" + m[i]['yk'] + "</td>";
				str += "</tr>";
				zuserje += Number(m[i]['userje']);
				zuserpoints += Number(m[i]['userpoints']);
				zzs += Number(m[i]['zs']);
				zupje += Number(m[i]['upje']);
				zzhong += Number(m[i]['zhong']);
				zshui += Number(m[i]['points']);
				zyk += Number(m[i]['yk'])
			}
			str += "<tr><Th>合计</th><td>" + getResult(zzs, 0) + "</td><td>" + getResult(zuserje, 2) + "</td><td>" + getResult(zuserpoints, 2) + "</td><td colspan=2></td><td class='zanbis'>" + getResult(zupje, 2) + "</td>";
			str += "<td>" + getResult(zshui, 2) + "</td>";
			str += "<td class='b1 sy'>" + getResult(zyk, 2) + "</td>";
			str += "</tr>";
			$(".baotb").html(str);
			$(".zanbi").each(function(){
			...
			for (i = 0; i < gbl; i++) {
				...
				zuserpoints = 0;
				zyk = 0;
				str += "<tr><td style='height:10px;' colspan=9></td></tr>";
				str += "<tr class='" + m[i][0]['style'] + "tb'><td colspan=9 class='bt' >" + m[i][0]['gname'] + "</td></tr><tr><th>公司</th><th>注数</th><th>会员下注</th><th>会员退水</th><th>贡献率</th><th>占比</th><th>集团占成</th><th>退水</th><th>集团收付</th></tr>";
				for (j = 0; j < ul; j++) {
					if (Number(m[i][j]['userzs']) == 0) continue;
					str += "<tr gid='" + m[i][0]['gid'] + "'  fly='" + m[i]['fly'] + "'>";
					str += "<td><a href='javascript:void(0);' class='u' fid=0 ifagent='" + m[i][j]['ifagent'] + "' uid='" + m[i][j]['userid'] + "' layer='" + m[i][j]['layer'] + "'  fly='" + m[i][j]['fly'] + "' >" + m[i][j]['username'] + "</a></td>";
					str += "<td>" + m[i][j]['userzs'] + "</td>";
					str += "<td>" + m[i][j]['userje'] + "</td>";
					str += "<td>" + m[i][j]['userpoints'] + "</td>";
					str += "<td class='gxl'>"+baifen(m[i][j]['userje'],m[i][j]['upje'])+"</td>";
					str += "<td class='zanbi' v2='"+m[i][j]['upje']+"'>0%</td>";					
					str += "<td >" + m[i][j]['upje'] + "</td>";
					str += "<td>" + m[i][j]['points'] + "</td>";
					str += "<td class='sy'>" + m[i][j]['yk'] + "</td>";
					str += "</tr>";
					zuserje += Number(m[i][j]['userje']);
					zuserpoints += Number(m[i][j]['userpoints']);
					zzs += Number(m[i][j]['zs']);
					zupje += Number(m[i][j]['upje']);
					zzhong += Number(m[i][j]['zhong']);
					zshui += Number(m[i][j]['points']);
					zyk += Number(m[i][j]['yk'])
				}
				str += "<tr><Th>合计</th><td>" + getResult(zzs, 0) + "</td><td>" + getResult(zuserje, 2) + "</td><td>" + getResult(zuserpoints, 2) + "</td><td colspan=2></td><td class='zanbis'>" + getResult(zupje, 2) + "</td>";
				str += "<td>" + getResult(zshui, 2) + "</td>";
...
function getbao() {
	...
	$.ajax({
		...
		success: function(m) {
			...
			var zshui = 0;
			var zyk = 0;
			str += "<tr><th>公司</th><th>注数</th><th>会员下注</th><th>会员退水</th><TH>贡献率</TH><TH>占比</TH><th>集团占成</th><th>退水</th><th>集团收付</th></tr>";
			var zuserje = 0;
			var zuserpoints = 0;
			for (i = 0; i < ml; i++) {
				str += "<tr>";
				str += "<td><a href='javascript:void(0);' class='u' fid=0 ifagent='" + m[i]['ifagent'] + "' uid='" + m[i]['userid'] + "' layer='" + m[i]['layer'] + "' ttype='" + m[i]['ttype'] + "'  >" + m[i]['username'] + "</a></td>";
				str += "<td>" + m[i]['userzs'] + "</td>";
				str += "<td>" + m[i]['userje'] + "</td>";
				str += "<td>" + m[i]['userpoints'] + "</td>";
					str += "<td class='gxl'>"+baifen(m[i]['userje'],m[i]['upje'])+"</td>";
