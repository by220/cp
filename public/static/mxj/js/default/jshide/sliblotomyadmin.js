...
function myready() {
	$(".user").find("option:first").html("选择" + layernames[0]['layer'][layer]);
	$("label").addClass('red');
	...
	$(".download").click(function() {
		var qishu = $("#qishu").val();
		$("#downfrm").attr('src', "nowloto?xtype=download&qishu=" + qishu)
	});
	addchange();
	$("#reload").click(function() {
		parent.tops.location.href = 'top?xtype=this';
		getnow()
	});
	$(".nowtb .bt").append("<th>所属" + layername[0] + "</th>");
...
function addchange() {
	...
	$(".user").change(function() {
		...
		var wid = $(this).find("option:selected").attr("wid");
		if (wid != '' & wid != undefined) {
			for (k = 0; k < layernames.length; k++) {
				if (Number(wid) == layernames[k]['wid']) {
					maxlayer = layernames[k]['layer'].length;
					layername = layernames[k]['layer']
				}
			}
		}
		if ((wid == '' | wid == undefined) & layer == toplayer + 1) {
