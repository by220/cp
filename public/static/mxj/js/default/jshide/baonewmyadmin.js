		...
		success: function(m) {
			...
			if (qishu == '') {
				if (end != start) {
					...
					$(".main3 .title").html( "（" + nowusername + "） - 交收报表 [" + start + "] ")
				}
			} else {
			...
			for (j = 0; j < ml; j++) {
				...
				if (qishu == '') {
					if (start != end) {
						...
						obj.find(".title").html( "（" + nowusername + "） - 交收报表 [" + start + "] ")
					}
				} else {
		...
		success: function(m) {
			...
			if (qishu == '') {
				if (start != end) {
					...
					$(".main1 .title").html(nowlayername + "（" + nowusername + "） - 分类报表 [" + start + "] ")
				}
			} else {
...
function agentnew() {
	...
	$.ajax({
		...
		success: function(m) {
			...
			if (qishu == '') {
				if (start != end) {
					$(".main2 .title").html(nowlayername + "（" + nowusername + "） - 交收报表 [" + start + " — " + end + "] ")
				} else {
					$(".main2 .title").html(nowlayername+ "（" + nowusername + "） - 交收报表 [" + start + "] ")
				}
			} else {
				$(".main2 .title").html(nowlayername + "（" + nowusername + "） - 交收报表 [" + $("input[name='lottery']:checked").attr('gname') + " — " + qishu + "期] ")
			}
			...
			for (i = 0; i < ml; i++) {
                ...
                str += "<td class='result color ff'>" + m[i]['sendyk_i'] + "</td>";
				if(nowlayer>0){
				    str += "<td>" + m[i]['sendje'] + "</td>";
	...
	$.ajax({
		...
		success: function(m) {
			...
			if(nowlayer>0){
				...
				if (qishu == '') {
					if (start != end) {
						obj.find(".title").html(nowlayername + "（" + nowusername + "） - 交收报表 [" + start + " — " + end + "] ")
					} else {
						obj.find(".title").html(nowlayername + "（" + nowusername + "） - 交收报表 [" + start + "] ")
					}
				} else {
					obj.find(".title").html(nowlayername + "（" + nowusername + "） - 交收报表 [" + $("input[name='lottery']:checked").attr('gname') + " — " + qishu + "期] ")
				}
				...
				for (i = 0; i < bl; i++) {
                    ...
                    str += "<td class='result color ff'>" + bao[i]['sendyk_i'] + "</td>";
					if(nowlayer>0){
