...
function myready() {
	$("#adduser").val("添加" + layername[0]);
	$("input.edit").click(function() {
		var uid = $(this).parent().parent().find("input:checkbox").attr('value');
		window.location.href = "user?xtype=editfirst&uid=" + uid;
		return false
	});
	...
	$.ajax({
		...
		success: function(m) {
			...
			$(".showdown").click(function() {
				...
				var wid = $(this).parent().parent().find("input:checkbox").attr('wid');
				for (k = 0; k < layernames.length; k++) {
					if (layernames[k]['wid'] == Number(wid)) {
						str = '';
						layername = new Array();
						for (g = 0; g < layernames[k]['layer'].length; g++) {
							str += "<option value='" + (g + 1) + "'>" + layernames[k]['layer'][g] + "</option>";
							layername[g] = layernames[k]['layer'][g]
						}
						$("#layer").html(str);
						str = null
					}
				...
				$("#layer").val(Number(ulayer) + 1);
				$("#adduser").val("添加" + $("#layer").find("option:selected").text());
				getuser();
				gettree();
				return false
			});
			...
			$(".setpoints").click(function() {
				...
				var uid = $(this).parent().parent().find("input:checkbox").val();
				window.location.href = 'suser?xtype=editson&uid=' + uid;
				return false
			});
...
function editfunc() {
	...
	$(".pantype").click(function() {
		...
		if (!flag & checked) {
			$("#defaultpan").append("<option value='" + pan + "'>" + pan + "</option>") $("#defaultpan").val($("#defaultpan option:first").val())
		}
	});
...
function addfunc() {
	...
	$("#ifagent").change(function() {
		if (Number($(this).val()) == 0) {
			...
			$(".flytype").attr("disabled", true);
			$(".zc").attr("disabled", true);
			$(".zchold").attr("disabled", true) $("#zchold").attr("disabled", true);
			$("#zc").attr("disabled", true);
			$("#flytype").attr("disabled", true)
		} else {
			$("#maxren").attr('disabled', false);
